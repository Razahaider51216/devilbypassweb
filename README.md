...
2. In OAuth2, add `http://localhost:3000/auth/discord/callback` for local development. For
   production, add `https://devilbypass.dev/auth/discord/callback` as a Redirect URI.
3. Copy the Client ID and Client Secret into `DISCORD_CLIENT_ID` and `DISCORD_CLIENT_SECRET` in `.env`.
4. Set `DISCORD_REDIRECT_URI` to that same URI. Production uses
   `PUBLIC_SITE_URL=https://devilbypass.dev` and `DISCORD_REDIRECT_URI=/auth/discord/callback`.
